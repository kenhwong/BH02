...
namespace BH02
{
    ...
    public class CurtainPanelInfo : ElementInfoBase
    {
        ...
        public CurtainPanelInfo(Element p, bool ispanel) : this()
        {
            ...
            INF_ErrorInfo = $"{p.Id}";
            Parameter _param;
            if ((_param = p.LookupParameter("构件分项")).HasValue) INF_Type = _param.AsInteger();
            else
            {
    ...
    public class ZoneInfoBase : INotifyPropertyChanged
    {
        ...
        {
            if (!System.Text.RegularExpressions.Regex.IsMatch(zcode, @"Z-\d{2}-\d{2}-[S|N|E|W][A-Z]-\d{2}")) throw new FormatException("Invalid zone code format while Initializing ZoneInfoBase instance.");
